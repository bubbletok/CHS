...
전투 모듈에서 총기·미사일이 타겟을 정확히 조준하고 있는지 판단하는 `IsYawFacingToTarget`/`IsPitchFacingToTarget` 로직이 있었는데, 실제 플레이에서 타겟을 제대로 바라보지 못하고 빗나가는 현상이 있었다. 코드를 들여다보니 각도 계산 시 평면 투영을 빠뜨린 게 원인이었다. Yaw는 `transform.forward`를 XZ 평면에 투영하지 않은 채 각도를 계산했고, Pitch는 기준 평면으로 로컬 좌표계(`transform.right`) 대신 월드 좌표계(`Vector3.right`)를 쓴 데다 `transform.forward`도 투영 없이 비교하고 있었다. 각각 `Vector3.ProjectOnPlane`로 올바른 평면에 투영한 뒤 비교하도록 고쳐 오차를 없앴다.

투영 오류를 고친 뒤에도 타겟과의 거리가 멀어질수록 고정된 각도 tolerance로는 계속 빗나가는 문제가 남아 있었다. 같은 각도 오차라도 거리가 멀수록 실제 위치 오차는 커지기 때문이었다. `atan(허용 오차 거리 / 타겟까지 거리)`로 거리에 따라 tolerance를 동적으로 좁히거나 넓히는 함수를 추가하고, 판정 방식도 방향 벡터를 직접 비교하던 방식에서 부모 좌표계 기준 raw yaw/pitch 값과 현재 회전값의 차이를 정규화해 비교하는 방식으로 바꿨다. 이 유틸리티는 적 탐지 로직에도 그대로 재사용해 중복 코드를 줄이고 탐지 정확도도 함께 개선했다. 타겟 방향으로 회전할 때 급격하게 꺾이던 문제도 AnimationCurve로 회전 속도를 조절해 자연스럽게 돌아가도록 다듬었다.
