...
처음 Jenkins를 다루었기에 `Jenkinsfile`을 통째로 만들어서 프로젝트마다 복사·붙여넣기하여 사용했다. 그러다 `Shared Library`라는 기능을 알게 되어 유니티 빌드나 `git checkout`, 배포 등의 작업들을 모듈화해서 분리하고 `Jenkinsfile`에는 최소한의 세팅만 남겨두도록 간소화했다.
