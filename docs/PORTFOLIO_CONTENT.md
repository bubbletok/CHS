...
LiDAR 포인트클라우드를 메시지로 변환할 때 GC Alloc이 발생했다.
`ToArray()` 대신 `NativeArray.Reinterpret` + `CopyTo`로 바꿔 할당을 줄였고,
GC 스파이크 때문에 생기던 프레임 드랍도 사라졌다.
다중 카메라 렌더링 병목은 DX12의 카메라별 RenderGraph · 리소스 배리어 비용까지
벤치마크로 확인한 뒤 DX11 전환으로 풀었다.

### ARCH — 구조 설계 · `상속 → 합성`

SOLID 원칙에 맞춰 리팩토링해 각 클래스가 단일 책임만 갖도록 정리했다.
`IImpactable` · `IExplosive` · `IExpirable` · `IManeuver` 인터페이스로
충돌 · 폭발 · 수명 · 기동 책임을 분리하고 오브젝트 풀로 관리해
기능 개발과 UI 개발을 충돌 없이 병렬로 진행했다.
