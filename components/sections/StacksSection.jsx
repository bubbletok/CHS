...
export default function StacksSection() {
  return (
    <section className="flex h-[calc(100vh-4rem)] w-full flex-col overflow-hidden px-5 py-8 sm:px-8 sm:py-10 lg:px-14">
      <Reveal className="flex w-full flex-1 flex-col overflow-hidden">
        <div className="grid min-h-0 flex-1 gap-6 lg:grid-cols-[minmax(0,17rem)_minmax(0,1fr)] lg:gap-16">
          ...
          <div className="flex shrink-0 flex-col gap-4">
            ...
            <h2 className="display text-[44px] sm:text-[64px] lg:text-[72px]">Stacks</h2>
          </div>
          ...
          <div className="min-h-0 overflow-y-auto lg:pr-4">
            {groups.map((g) => (
              <section key={g.id} className="pb-10">
                ...
                <div className="sticky top-0 z-10 flex items-baseline justify-between gap-4 border-b-2 border-ink bg-bg pb-2 pt-3">
                ...
                <ul className="mt-2">
                  {g.items.map((item) => (
                    <li
                      key={item.name}
                      className={`group flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 border-b border-line/40 pl-4 transition-all duration-200 hover:pl-6 sm:pl-5 sm:hover:pl-7 ${
                        item.key ? 'border-l border-ink py-3.5' : 'py-3'
                      }`}
                    >
                      <span
                        className={
                          item.key
                            ? 'text-xl font-semibold leading-tight tracking-tight text-ink sm:text-2xl'
                            : 'text-base text-muted transition-colors group-hover:text-ink sm:text-lg'
                        }
                      >
                        {item.name}
                      </span>
